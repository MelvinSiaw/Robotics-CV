...
# GPIO setup
servo_x_pin = 18  # GPIO pin for servo controlling X
servo_y_pin = 23  # GPIO pin for servo controlling Y

GPIO.setmode(GPIO.BCM)
GPIO.setup(servo_x_pin, GPIO.OUT)
GPIO.setup(servo_y_pin, GPIO.OUT)

# Create PWM instances for X and Y servos
...
def set_servo_angle(servo, angle):
    # Convert angle to duty cycle (0 to 100)
    duty_cycle = angle / 18.0 + 2.5
    servo.ChangeDutyCycle(duty_cycle)
    time.sleep(0.2)  # Allow servo to reach the position
...
def main():
    ...
    while True:
        ...
        # If X and Y positions are available, control the servos
        if current_position_x is not None and current_position_y is not None:
            set_servo_angle(servo_x, current_position_x)
            set_servo_angle(servo_y, current_position_y)

    cap.release()
    cv2.destroyAllWindows()
    GPIO.cleanup()  # Clean up GPIO resources when the program exits
